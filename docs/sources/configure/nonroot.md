...
Some components like [beyla.ebpf][] require elevated privileges. Running {{< param "PRODUCT_NAME" >}} as a non-root user with `beyla.ebpf` requires setting file capabilities on the {{< param "PRODUCT_NAME" >}} binary. Refer to [beyla.ebpf permissions][] for details.
