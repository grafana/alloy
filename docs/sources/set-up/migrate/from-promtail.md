...
* If you are converting a promtail configuration for a kubernetes daemonset, you will likely want to [modify the generated configuration][single-node-discovery] to ensure `discovery.kubernetes` only discovers pods residing on the same node as the {{< param "PRODUCT_NAME" >}} pod.
