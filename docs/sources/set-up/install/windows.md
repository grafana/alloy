...
- `ALLOY_OTEL_MODE`: Type `REG_SZ`. Set this value to `1` or `true` (case-insensitive) to run the {{< param "OTEL_ENGINE" >}} instead of the {{< param "DEFAULT_ENGINE" >}}. Refer to [Run the {{< param "OTEL_ENGINE" >}} with a service installation][OTelEngine] for more information.
- `OTelArguments`: Type `REG_MULTI_SZ`. Each value represents a binary argument for the {{< param "PRODUCT_NAME" >}} binary when it runs the {{< param "OTEL_ENGINE" >}}. {{< param "PRODUCT_NAME" >}} ignores this value when it runs the {{< param "DEFAULT_ENGINE" >}}.
...
[OTelEngine]: ../../otel_engine/service-installation/
[Run]: ../../run/windows/
