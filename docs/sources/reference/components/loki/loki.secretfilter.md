...
It's disabled by default because it can generate false positives. Consider enabling entropy-based filtering if you enable this rule.
...
The `enable_entropy` argument enables entropy-based filtering. When set to `true`, the entropy of the detected potential secret is calculated and compared against the threshold provided for the matching rule in the configuration file. The secret is then redacted only if the entropy is above the threshold. This can help reduce false positives.
