...
| `on_positions_file_error` | `string`             | How to handle a corrupt positions file entry for a given file. | `"skip"` | no       |
...
`"skip"` will cause the tailer to skip the file and not collect logs. This is the default behavior because it represents the previous behavior 
