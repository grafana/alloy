...
| `ignore_uuid` | `bool`   | When true, UUID-formatted strings are never redacted.          | `false`          | no       |
...
When `ignore_uuid` is `true`, any string matching the standard UUID format (`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`) is excluded from Luhn evaluation.
This is useful when log lines contain UUIDs whose digit groups happen to pass the Luhn check, which would otherwise cause them to be incorrectly redacted.
...
Without `ignore_uuid`, the stage would incorrectly redact part of the UUID.
