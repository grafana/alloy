...
| `action_on_duplicate_timestamp` | `string`       | What to do when the parsed timestamp equals (or is before) the last emitted timestamp for that stream. | `"fudge"` | no       |
...
* fudge (default): Change the timestamp to the last known timestamp, plus 1 nanosecond, to preserve log entry ordering.
