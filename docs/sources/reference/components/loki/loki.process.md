...
| Name                     | Type                    | Description                                                 | Default | Required |
| ------------------------ | ----------------------- | ----------------------------------------------------------- | ------- | -------- |
| `forward_to`             | `list(MetricsReceiver)` | Receivers to forward metrics to instead of exposing locally.|         | no       |
| `metrics_flush_interval` | `duration`              | Frequency to flush metrics to the forwarded receivers.      | `"60s"` | no       |

When you provide `forward_to`, the `stage.metrics` block forwards generated metrics to the specified receivers, such as `prometheus.remote_write`, instead of exposing them at the {{< param "PRODUCT_NAME" >}} `/metrics` endpoint.
This allows you to send derived metrics directly to a remote storage backend without exposing them locally.

The `metrics_flush_interval` argument controls how often the component flushes metrics to the receivers you specify in `forward_to`. This argument's only used when you set `forward_to`.

{{< admonition type="note" >}}
When you use `forward_to`, the component doesn't append the metric type and description to the remote write request because the remote write protocol doesn't support updating metadata for appended metrics.
{{< /admonition >}}
...
In this example, `prometheus.remote_write` sends the `loki_log_metrics_successful_orders_total` counter metric directly to the remote Prometheus instance instead of exposing it at the {{< param "PRODUCT_NAME" >}} `/metrics` endpoint.
