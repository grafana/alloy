...
1. Use an [Azure Resource Graph](https://azure.microsoft.com/en-us/get-started/azure-portal/resource-graph/#overview) query to identify resources for gathering metrics. This is the default option.
...
- When using an Azure Resource Graph query, [read access to the resources that Resource Graph queries](https://learn.microsoft.com/en-us/azure/governance/resource-graph/overview#permissions-in-azure-resource-graph).
...
<!-- vale Grafana.GoogleSpacing = YES -->
If a single dimension is requested, it has the name `dimension`.
If multiple dimensions are requested, they have the name `dimension<dimension_name>`.

Tags in `included_resource_tags` are added as labels with the name `tag_<tag_name>`.
...
When `validate_dimensions` is enabled, you need one exporter instance per metric + dimension combination, which is more tedious to maintain.
