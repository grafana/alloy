...
`sigil.write` receives parsed [Grafana AI Observability](https://grafana.com/docs/grafana-cloud/machine-learning/ai-observability/) generation records from other components and forwards them to remote Sigil ingest endpoints.
The records use the wire format produced by the [Grafana AI Observability SDK](https://github.com/grafana/sigil-sdk).
...
The `sigil.write` component doesn't support any arguments. You can configure this component with blocks.
...
| -------------------------------------------------- | ---------------------------------------------------------- | -------- |
| [`endpoint`][endpoint]                             | Location to send generations to.                           | yes      |
...
For example, `endpoint` > `authorization` refers to an `authorization` block defined inside an `endpoint` block.
...
| `headers`                | `map(string)`       | Extra headers to deliver with the request. `Content-Type` is always set to `application/json` and cannot be overridden. |           | no       |
| `http_headers`           | `map(list(secret))` | Custom HTTP headers to be sent along with each request. The map key is the header name.          |           | no       |
| `insecure`               | `bool`              | Use `http` instead of `https` when `url` does not include a scheme.                              | `false`   | no       |
...
The `tenant_id` argument overrides any `X-Scope-OrgID` value from `headers` or the incoming request.
If `tenant_id` is empty, `sigil.write` uses `X-Scope-OrgID` from `headers` when set, and then falls back to the incoming request tenant.
...
Requests are retried on HTTP 429, 408, and 5xx responses with exponential backoff. Other non-2xx responses aren't retried.
...
`sigil.write` doesn't expose any component-specific debug information.
