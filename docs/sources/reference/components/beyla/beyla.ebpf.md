...
When `native_histograms` is set to `true`, the component serves metrics in OpenMetrics format.
This allows a [`prometheus.scrape` component][scrape] with `scrape_native_histograms = true` to collect native histograms alongside classic histograms.
