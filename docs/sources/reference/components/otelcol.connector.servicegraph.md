...
`virtual_node_peer_attributes` | `list(string)`   | The list of attributes, ordered by priority, whose presence in a client span will result in the creation of a virtual server node. An empty list disables virtual node creation. | `["peer.service", "db.name", "db.system"]` | no
