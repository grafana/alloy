...
`otelcol.receiver.syslog` doesn't support any arguments and is configured fully
...
To send telemetry signals to `otelcol.receiver.syslog` with HTTP/JSON, POST to:
...
otelcol.processor.batch "default" {
  output {
    metrics = [otelcol.exporter.syslog.default.input]
    logs    = [otelcol.exporter.syslog.default.input]
    traces  = [otelcol.exporter.syslog.default.input]
  }
}
...
`otelcol.receiver.syslog` supports [gzip](https://en.wikipedia.org/wiki/Gzip) for compression.
