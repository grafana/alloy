...
{{< docs/shared lookup="stability/generally-available.md" source="alloy" version="<ALLOY_VERSION>" >}}
...
`otelcol.exporter.file` is a wrapper over the upstream OpenTelemetry Collector Contrib [`fileexporter`][] exporter. Bug reports or feature requests might be redirected upstream.
...
otelcol.exporter.file "LABEL" {
  path = "/tmp/<FILENAME>.json" // Replace <FILENAME> with the desired output file name
}
```
...
| `compression` | `string` | Compression algorithm. Only `"zstd"` is supported. |  | no |
| `flush_interval` | `duration` | Time between flushes to disk. Must be greater than zero. | `"1s"` | no |
| `encoding` | `string` | (Unsupported) Encoding extension ID. Setting this returns a validation error. |  | no |

{{< admonition type="caution" >}}
You cannot enable `append` and `compression` together.
You cannot enable `append` and `rotation` together.
The `encoding` argument is currently not supported. Use `format` instead.
{{< /admonition >}}
...
| ----- | ----------- | -------- |
| [`rotation`][rotation] | Configures file rotation. Ignored when `group_by.enabled = true`. | no |
...
| `max_days` | `int` | Maximum number of days to retain old files. `0` keeps files indefinitely. | `0` | no |
| `max_backups` | `int` | Maximum number of old rotated files to retain. | `100` | no |
...
`otelcol.exporter.file` does not export any fields.
...
`otelcol.exporter.file` does not expose any component-specific debug information.
...
`otelcol.exporter.file` is a wrapper over the upstream OpenTelemetry Collector Contrib [`fileexporter` exporter](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/exporter/fileexporter).

The component writes telemetry data to files on disk in either JSON or Protocol Buffers format. It supports:
