...
| `max_request_body_size` | `string`            |  Maximum request body size the receiver accepts, applied to the raw request body and again to the payload after gzip decompression. The receiver rejects larger requests with HTTP 422. Must be greater than `0`. | `"20MiB"`              | no       |
