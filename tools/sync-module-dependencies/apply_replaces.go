...
func main() {
	...
	replacesPath := filepath.Join(scriptDir, "dependency-replacements.yaml")
	...
	for _, module := range projectReplaces.Modules {
		if err := applyReplacesToModule(scriptDir, module); err != nil {
			log.Fatalf("Failed to apply replaces to module %q: %v", module.Name, err)
		}
	}
}

func applyReplacesToModule(scriptDir string, module types.Module) error {
	...
	case "yaml":
		...
		endMarker = "  # END GENERATED REPLACES"
	default:
		return fmt.Errorf("unknown file_type %q for module %q (expected 'mod' or 'yaml')", module.FileType, module.Name)
	}
