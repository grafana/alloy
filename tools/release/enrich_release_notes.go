...
type enrichReleaseNotesFlags struct {
	tag        string
	footerFile string
	dryRun     bool
}
...
func enrichReleaseNotesCommand() *cobra.Command {
	var flags enrichReleaseNotesFlags

	cmd := &cobra.Command{
		Use:   "enrich-release-notes",
		Short: "Add contributor attribution and an optional footer to a release's notes",
		RunE: func(cmd *cobra.Command, args []string) error {
			return runEnrichReleaseNotes(cmd.Context(), flags)
		},
	}
...
func runEnrichReleaseNotes(ctx context.Context, flags enrichReleaseNotesFlags) error {
