...
// TestCapabilities asserts that Alloy with an empty configuration requests no Linux capabilities at all.
// Ideally, Alloy should not request any capabilities at all.
func TestCapabilities(t *testing.T) {
	containerID := os.Getenv(common.TetragonContainerIDEnv)
	if containerID == "" {
		t.Fatalf("Tetragon container not configured (tetragon_container.image not set in test.yaml)")
	}
