package databaseobservabilitymysql
...
func TestDatabaseObservabilityMySQL(t *testing.T) {
	ns := deps.NewNamespace(deps.NamespaceOptions{
		Name:   "test-database-observability-mysql",
		Labels: map[string]string{"alloy-integration-test": "true"},
	})
	mysql := deps.NewMySQL(deps.MySQLOptions{Namespace: ns.Name()})
	...
	alloy := deps.NewAlloy(deps.AlloyOptions{
		Namespace:  ns.Name(),
		Release:    "alloy-test-dbo-mysql",
		ConfigPath: "./config/config.alloy",
		ValuesPath: "./config/alloy-values.yaml",
	})
	harness.Setup(t, harness.Options{
		Dependencies: []harness.Dependency{ns, mysql, mimir, loki, alloy},
	})

	const testName = "database-observability-mysql"
