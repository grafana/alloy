...
func TestConvert(t *testing.T) {
	test_common.TestDirectory(t, "testdata", ".yaml", true, []string{"-config.expand-env"}, map[string]struct{}{}, staticconvert.Convert, false)
	test_common.TestDirectory(t, "testdata-v2", ".yaml", true, []string{"-enable-features", "integrations-next", "-config.expand-env"},
		map[string]struct{}{}, staticconvert.Convert, false)
	test_common.TestDirectory(t, "testdata_linux", ".yaml", true, []string{"-config.expand-env"}, map[string]struct{}{}, staticconvert.Convert, false)
}
