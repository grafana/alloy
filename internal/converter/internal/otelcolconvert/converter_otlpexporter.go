...
func toQueueArguments(cfg configoptional.Optional[exporterhelper.QueueBatchConfig]) otelcol.QueueArguments {
	if !cfg.HasValue() {
		// When no queue config is specified in the source, return Alloy's default
		// (queue enabled by default) to preserve backwards compatibility.
		return common.DefaultValue[otelcol.QueueArguments]()
	}
