...
func TestMetricsPipeline(t *testing.T) {
	...
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testMetricYaml), nil, registry)
	if err != nil {
		t.Fatal(err)
	}
...
`
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testConfig), nil, registry)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMetricsWithDropInPipeline(t *testing.T) {
	registry := prometheus.NewRegistry()
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testMetricWithDropYaml), nil, registry)
	if err != nil {
		t.Fatal(err)
	}
	lbls := model.LabelSet{}
...
`
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			registry := prometheus.NewRegistry()
			pl, err := NewPipeline(log.NewNopLogger(), loadConfig(tc.promtailConfig), nil, registry)
...
func TestDefaultIdleDuration(t *testing.T) {
	...
	ms, err := New(log.NewNopLogger(), nil, StageTypeMetric, metricsConfig, registry)
	if err != nil {
...
func TestMetricStage_Process(t *testing.T) {
	...
	registry := prometheus.NewRegistry()
	jsonStage, err := New(log.NewNopLogger(), nil, StageTypeJSON, jsonConfig, registry)
	if err != nil {
		t.Fatalf("failed to create stage with metrics: %v", err)
	}
	regexStage, err := New(log.NewNopLogger(), nil, StageTypeRegex, regexConfig, registry)
	if err != nil {
		t.Fatalf("failed to create stage with metrics: %v", err)
	}
	metricStage, err := New(log.NewNopLogger(), nil, StageTypeMetric, metricsConfig, registry)
	if err != nil {
		t.Fatalf("failed to create stage with metrics: %v", err)
	}
