...
func TestEventLogMessage_simple(t *testing.T) {
	...
	for testName, testData := range tests {
		...
		t.Run(testName, func(t *testing.T) {
			...
			pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testData.config), nil, prometheus.DefaultRegisterer)
...
func TestEventLogMessageConfig_validate(t *testing.T) {
	...
	for tName, tt := range tests {
		t.Run(tName, func(t *testing.T) {
			_, err := newEventLogMessageStage(log.NewNopLogger(), tt.config)
			if tt.err != nil {
...
func TestEventLogMessage_invalidString(t *testing.T) {
	...
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testEvtLogMsgYamlDefaults), nil, prometheus.DefaultRegisterer)
	assert.NoError(t, err, "Expected pipeline creation to not result in error")
	out := processEntries(pl,
