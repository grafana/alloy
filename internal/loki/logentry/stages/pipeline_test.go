...
func TestNewPipeline(t *testing.T) {
	p, err := NewPipeline(log.NewNopLogger(), loadConfig(testMultiStageYaml), nil, prometheus.DefaultRegisterer)
	if err != nil {
		panic(err)
	}
...
func TestPipeline_Process(t *testing.T) {
	...
	for tName, tt := range tests {
		t.Run(tName, func(t *testing.T) {
			...
			p, err := NewPipeline(log.NewNopLogger(), config["pipeline_stages"].([]interface{}), nil, prometheus.DefaultRegisterer)
			require.NoError(t, err)
...
func newPipelineFromConfig(cfg, name string) (*Pipeline, error) {
	...
	return NewPipeline(log.NewNopLogger(), config["pipeline_stages"].([]interface{}), &name, prometheus.DefaultRegisterer)
}
