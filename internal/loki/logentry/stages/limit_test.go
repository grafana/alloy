...
func TestLimitWaitPipeline(t *testing.T) {
	...
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testLimitWaitYaml), &plName, registry)
	logs := make([]Entry, 0)
	logCount := 5
...
func TestLimitDropPipeline(t *testing.T) {
	registry := prometheus.NewRegistry()
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testLimitDropYaml), &plName, registry)
	logs := make([]Entry, 0)
	logCount := 10
	for i := 0; i < logCount; i++ {
		logs = append(logs, newEntry(nil, model.LabelSet{"app": "loki"}, testMatchLogLineApp1, time.Now()))
...
func TestLimitByLabelPipeline(t *testing.T) {
	registry := prometheus.NewRegistry()
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testLimitByLabelYaml), &plName, registry)
	logs := make([]Entry, 0)
	logCount := 5
	for i := 0; i < logCount; i++ {
		logs = append(logs, newEntry(nil, model.LabelSet{"app": "loki"}, testMatchLogLineApp1, time.Now()))
