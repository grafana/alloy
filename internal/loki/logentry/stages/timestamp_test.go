...
func TestTimestampPipeline(t *testing.T) {
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testTimestampYaml), nil, prometheus.DefaultRegisterer)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTimestampStage_Process(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			st, err := newTimestampStage(log.NewNopLogger(), test.config)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTimestampStage_ProcessActionOnFailure(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			s, err := newTimestampStage(log.NewNopLogger(), testData.config)
