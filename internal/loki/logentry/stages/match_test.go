...
func TestMatchPipeline(t *testing.T) {
	...
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testMatchYaml), &plName, registry)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMatcher(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			s, err := newMatcherStage(log.NewNopLogger(), nil, matchConfig, prometheus.DefaultRegisterer)
