...
import (
	...
	"github.com/grafana/alloy/internal/loki/promtail/client/fake"
...
func Test_newReaderTarget(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			scrapeconfig.Config{},
			[]api.Entry{
				{Labels: model.LabelSet{}, Entry: push.Entry{Line: "bar"}},
			},
			false,
		},
		{
		...
		},
		{
			...
			scrapeconfig.Config{},
			[]api.Entry{
				{Labels: model.LabelSet{}, Entry: push.Entry{Line: "foo"}},
				{Labels: model.LabelSet{}, Entry: push.Entry{Line: "bar"}},
			},
			false,
		},
		{
			...
			[]api.Entry{
				{Labels: model.LabelSet{"new_key": "hello world!"}, Entry: push.Entry{Line: "foo"}},
				{Labels: model.LabelSet{"new_key": "hello world!"}, Entry: push.Entry{Line: "bar"}},
			},
			false,
		},
		{
			...
			defaultStdInCfg,
			[]api.Entry{
				{Labels: model.LabelSet{"job": "stdin", "hostname": model.LabelValue(hostName)}, Entry: push.Entry{Line: "foo"}},
				{Labels: model.LabelSet{"job": "stdin", "hostname": model.LabelValue(hostName)}, Entry: push.Entry{Line: "bar"}},
			},
			false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := fake.New(func() {})
			got, err := newReaderTarget(prometheus.DefaultRegisterer, log.NewNopLogger(), tt.in, c, tt.cfg)
...
func Test_Shutdown(t *testing.T) {
	...
	manager, err := NewStdinTargetManager(prometheus.DefaultRegisterer, log.NewNopLogger(), appMock, recorder, []scrapeconfig.Config{{}})
	require.NoError(t, err)
	require.NotNil(t, manager)
	require.Equal(t, true, <-appMock.called)
	recorder.Stop()
	compareEntries(t, []api.Entry{{Labels: model.LabelSet{}, Entry: push.Entry{Line: "line"}}}, recorder.Received())
}
...
func Test_StdinConfigs(t *testing.T) {
	...
	require.Equal(t, scrapeconfig.DefaultScrapeConfig, getStdinConfig(log.NewNopLogger(), []scrapeconfig.Config{
		scrapeconfig.DefaultScrapeConfig,
		{},
	}))
	// or use the default if none if provided
	require.Equal(t, defaultStdInCfg, getStdinConfig(log.NewNopLogger(), []scrapeconfig.Config{}))
}
