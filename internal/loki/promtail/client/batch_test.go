...
func TestHashCollisions(t *testing.T) {
	...
	for i := 0; i < entriesPerLabel; i++ {
		_ = b.add(api.Entry{Labels: ls1, Entry: push.Entry{Timestamp: time.Now(), Line: fmt.Sprintf("line %d", i)}})

		_ = b.add(api.Entry{Labels: ls2, Entry: push.Entry{Timestamp: time.Now(), Line: fmt.Sprintf("line %d", i)}})
	}
