...
func (t targetBuilder) Target() Target {
	...
	if modifyGroup {
		// TODO(thampiotr): When relabeling a lot of targets we might produce a lot of t.groups that will
		//                  be the same. This could be remediated with an extra step to consolidate them using
		//					perhaps a hash as ID. See first if this is a problem.
