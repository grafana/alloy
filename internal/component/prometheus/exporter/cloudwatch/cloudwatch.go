...
func createExporter(opts component.Options, args component.Arguments) (integrations.Integration, string, error) {
	...
	if a.DecoupledScrape.Enabled {
		exp, err := cloudwatch_exporter.NewDecoupledCloudwatchExporter(opts.ID, opts.SLogger, exporterConfig, a.DecoupledScrape.ScrapeInterval, fipsEnabled, a.LabelsSnakeCase, a.Debug)
		return exp, getHash(a), err
	}

	exp, err := cloudwatch_exporter.NewCloudwatchExporter(opts.ID, opts.SLogger, exporterConfig, fipsEnabled, a.LabelsSnakeCase, a.Debug)
	return exp, getHash(a), err
}
