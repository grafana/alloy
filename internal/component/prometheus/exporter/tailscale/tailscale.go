...
type OAuthArguments struct {
	...
	AdvertiseTags []string `alloy:"advertise_tags,attr"`
}
...
func (a *Arguments) Validate() error {
	...
	case hasOAuth:
		...
		if len(a.OAuth.AdvertiseTags) == 0 {
			return fmt.Errorf("oauth.advertise_tags must contain at least one tag")
		}
		for idx, tag := range a.OAuth.AdvertiseTags {
			if !strings.HasPrefix(tag, "tag:") || len(tag) == len("tag:") {
				return fmt.Errorf("oauth.advertise_tags[%d] must start with \"tag:\" and include a tag name", idx)
			}
		}
	}
	...
	for idx, t := range a.Targets {
		...
		if t.Path != "" && !strings.HasPrefix(t.Path, "/") {
			return fmt.Errorf("target[%d].path must start with \"/\"", idx)
		}
		if len(t.MatchTags) == 0 && idx != len(a.Targets)-1 {
			return fmt.Errorf("target[%d] matches every device and must be the last target block", idx)
		}
		for patternIdx, pattern := range t.MatchTags {
			if _, err := path.Match(pattern, ""); err != nil {
				return fmt.Errorf("target[%d].match_tags[%d] is invalid: %w", idx, patternIdx, err)
			}
		}
		for name := range t.Labels {
			if name == "node" {
				return fmt.Errorf("target[%d].labels cannot override the reserved \"node\" label", idx)
			}
			if !model.LegacyValidation.IsValidLabelName(name) {
				return fmt.Errorf("target[%d].labels contains invalid label name %q", idx, name)
			}
		}
	}
	return nil
