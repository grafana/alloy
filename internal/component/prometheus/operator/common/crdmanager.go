...
func (c *crdManager) addScrapeConfig(pm *promopv1alpha1.ScrapeConfig) {
	...
	mapKeys := []string{}
	for i, ep := range pm.Spec.StaticConfigs {
		var scrapeConfig *config.ScrapeConfig
		scrapeConfig, err = gen.GenerateStaticScrapeConfigConfig(pm, ep, i)
		if err != nil {
			level.Error(c.logger).Log("name", pm.Name, "err", err, "msg", "error generating scrapeconfig from scrapeconfig")
			break
		}
		mapKeys = append(mapKeys, scrapeConfig.JobName)
		c.mut.Lock()
