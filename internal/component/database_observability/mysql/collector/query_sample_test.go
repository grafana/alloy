	...
	testcases := []struct {
	...
	}{
		{
			...
			rows: [][]driver.Value{{
				...
				"456",
				"457",
				"wait/io/file/innodb/innodb_data_file", // WAIT_EVENT_NAME
				"100000000",                            // WAIT_TIME
				"123",                                  // WAIT_NESTING_EVENT_ID
				"STATEMENT",                            // WAIT_NESTING_EVENT_TYPE
			}},
			logsLabels: []model.LabelSet{
				{"job": database_observability.JobName, "op": OP_QUERY_SAMPLE, "instance": "mysql-db"},
			},
			logsLines: []string{
				`level="info" schema="some_schema" event_id="123" digest="some_digest" digest_text="select * from some_table where id = :redacted1" rows_examined="5" rows_sent="5" rows_affected="0" errors="0" max_controlled_memory="456b" max_total_memory="457b" cpu_time="0.010000ms" elapsed_time="0.020000ms" elapsed_time_ms="0.020000ms" wait_event_name="wait/io/file/innodb/innodb_data_file" wait_time="0.100000ms" wait_nesting_event_id="123" wait_nesting_event_type="STATEMENT"`,
			},
		},
		{
			...
			rows: [][]driver.Value{{
				...
				nil, // WAIT_EVENT_NAME
				nil, // WAIT_TIME
				nil, // WAIT_NESTING_EVENT_ID
				nil, // WAIT_NESTING_EVENT_TYPE
			}},
			logsLabels: []model.LabelSet{
				{"job": database_observability.JobName, "op": OP_QUERY_SAMPLE, "instance": "mysql-db"},
			},
			...
			mock.ExpectQuery(fmt.Sprintf(selectQuerySamples, "", endOfTimeline)).WithArgs(
				...
				WillReturnRows(
					sqlmock.NewRows([]string{
						...
						"waits.event_name",
						"waits.timer_wait",
						"waits.NESTING_EVENT_ID",
						"waits.NESTING_EVENT_TYPE",
					}).AddRows(
						tc.rows...,
					),
				)
