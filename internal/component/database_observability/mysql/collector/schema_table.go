...
func (c *SchemaTable) extractSchema(ctx context.Context) error {
	...
	for _, table := range tables {
		...
		table, err = c.getTableDefinition(ctx, fullyQualifiedTable, table)
		if err != nil {
			level.Error(c.logger).Log("msg", "failed to get table definition", "schema", table.schema, "table", table.tableName, "err", err)
			continue
		}
...
func (c *SchemaTable) getTableDefinition(ctx context.Context, fullyQualifiedTable string, table *tableInfo) (*tableInfo, error) {
