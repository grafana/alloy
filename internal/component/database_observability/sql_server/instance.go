...
// databaseConfig is the normalized configuration of one monitored SQL Server
// instance, derived either from the component's top-level arguments
// (single-DSN form) or from one `database_instance` block.
type databaseConfig struct {
	...
	// form. It's always empty for `database_instance` blocks.
	targets []discovery.Target
}

// databaseConfigs returns one databaseConfig per monitored instance. When no
// `database_instance` blocks are defined, the top-level arguments describe a
...
type dbInstance struct {
	...
	// exportedTargets holds the relabeled targets of this instance. It's set
...
// metricsPath returns the HTTP path of an instance's metrics endpoint,
// relative to the component's root HTTP path. The unnamed instance of the
