...
func TestActivity_QueryRedaction(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedLines: []string{
				fmt.Sprintf(`level="info" instance="test" datname="testdb" pid="100" leader_pid="" user="testuser" app="testapp" client="127.0.0.1:5432" backend_type="client backend" backend_time="%s" xid="0" xmin="0" xact_time="%s" state="active" query_time="%s" queryid="128" query="INSERT INTO users (id, name, email) VALUES (?, ?, ?)" engine="postgres" cpu_time="%s"`,
					time.Duration(0).String(),
					time.Duration(0).String(),
					time.Duration(0).String(),
					time.Duration(0).String(),
