...
func TestQueryDetails(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			logsLabels: []model.LabelSet{
				{"op": database_observability.OP_QUERY_ASSOCIATION},
				{"op": database_observability.OP_QUERY_PARSED_TABLE_NAME},
			},
			logsLines: []string{
...
func TestQueryDetails_QueryAssociation_OnEmitsFingerprint(t *testing.T) {
	...
	fp, err := fingerprint.Fingerprint(q)
	require.NoError(t, err)
...
func TestQueryDetails_QueryAssociation_OffOmitsFingerprint(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		for _, e := range lokiClient.Received() {
			if e.Labels["op"] == database_observability.OP_QUERY_ASSOCIATION {
				return true
			}
		}
		return false
