...
// TestComplexInsertWithCTE validates fingerprinting on the most complex production query
// Real query from books_store database: queryid -2340733133787340944 (1257 characters)
func TestComplexInsertWithCTE(t *testing.T) {
	// Multiple CTEs, INSERT...RETURNING, CROSS JOIN, window functions, CASE statements, 18 parameters
	query := `WITH new_books AS (
		...
		SELECT
			...
			CASE
				WHEN random() < $7 THEN ($8 + floor(random() * $9))
				WHEN random() < $10 THEN ($11 + floor(random() * $12))
				ELSE ($13 + floor(random() * $14))
			END as stock,
			($15 + floor(random() * $16))::integer as category_id
		FROM gen_themes t CROSS JOIN gen_adjectives adj
		ORDER BY random()
		LIMIT $17
		RETURNING id
	),
	book_author_insertions AS (
		INSERT INTO book_authors (book_id, author_id)
		SELECT b.id, a.id
		FROM new_books b
		CROSS JOIN (SELECT id FROM authors OFFSET floor(random() * (SELECT COUNT(*) FROM authors)) LIMIT $18) a
		RETURNING book_id
	)
	SELECT DISTINCT nb.id FROM new_books nb`

	hash, err := pg_query.Fingerprint(query)
	require.NoError(t, err)
	assert.NotEmpty(t, hash, "Complex production query should produce a fingerprint")

	t.Logf("Most complex production query (1257 chars, 18 params) fingerprint: %s", hash)
}
