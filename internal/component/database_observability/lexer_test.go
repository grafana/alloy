...
func TestContainsReservedKeywords(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "SHOW GLOBAL STATUS LIKE literal",
		...
		{
			name:          "SHOW TABLES LIKE placeholder",
		...
		{
			name:          "KILL QUERY",
			query:         "KILL QUERY 123",
			reservedWords: ExplainReservedWordDenyList,
			expected:      true,
		},
		{
			name:          "SELECT with show_id column should not be detected",
			query:         "SELECT show_id FROM users",
			reservedWords: ExplainReservedWordDenyList,
			expected:      false,
		},
		{
			name:          "SELECT with kill in string literal should not be detected",
			query:         "SELECT 'kill query' FROM users",
			reservedWords: ExplainReservedWordDenyList,
			expected:      false,
		},
		{
			name:          "CALL stored procedure",
			query:         "CALL do_work(1, 2)",
			reservedWords: ExplainReservedWordDenyList,
			expected:      true,
		},
		{
		...
		{
			name:          "DO MySQL expression",
			query:         "DO SLEEP(1)",
			reservedWords: ExplainReservedWordDenyList,
			expected:      true,
		},
		{
			name:          "DO Postgres anonymous block",
			query:         "DO 'BEGIN PERFORM 1; END'",
			reservedWords: ExplainReservedWordDenyList,
			expected:      true,
		},
		{
			name:          "SELECT with call_count column should not be detected",
			query:         "SELECT call_count FROM users",
			reservedWords: ExplainReservedWordDenyList,
			expected:      false,
		},
		{
			name:          "SELECT with do_something column should not be detected",
			query:         "SELECT do_something FROM users",
			reservedWords: ExplainReservedWordDenyList,
			expected:      false,
		},
