...
func TestDeltaProfilesManager(t *testing.T) {
	...
	m, err := NewManager(Options{}, Arguments{}, pyroscope.AppendableFunc(func(ctx context.Context, lbs labels.Labels, samples []*pyroscope.RawSample) error {
	...
	targetSetsChan <- []*targetgroup.Group{{Targets: targets}}
	...
	require.Eventually(t, func() bool {
		...
		for _, metric := range metrics {
			if metric.GetName() == "pyroscope_delta_map_size2" {
				return len(metric.GetMetric()) > 0
			}
		}
		t.Logf("pyroscope_delta_map_size2 metric not found")
		return false
	...
	for _, mf := range metricFamilies {
		if mf.GetName() == "pyroscope_delta_map_size2" {
	...
	for _, m := range deltaMapSizeMetric.GetMetric() {
		...
		for _, label := range l {
			ns := label.GetValue()
			if label.GetName() == "target_namespace" {
