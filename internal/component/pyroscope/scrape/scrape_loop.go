...
func validateProfileData(data []byte, contentType string) error {
	...
	if isTextContent(data) {
		...
		return fmt.Errorf("received text-based error response instead of pprof: %q", preview)
	}
...
func validateContentType(contentType string) error {
	if contentType == "" {
		return fmt.Errorf("missing Content-Type header, expected application/octet-stream for pprof data")
	}
...
// hasHTMLPrefix checks for HTML prefixes
