...
func TestMultilineStreamsStriped(t *testing.T) {
	now := time.Now()

	// buffer appends line the way process does, so the map ends up holding
	// state a flush will collapse.
	buffer := func(m *multilineStreamsStriped, fp model.Fingerprint, line string, lastSeen time.Time) {
		m.Mutate(fp, func(s *multilineState, ok bool) *multilineState {
			if !ok {
				s = &multilineState{
					buffer:         new(bytes.Buffer),
					startLineEntry: newEntry(map[string]any{}, model.LabelSet{}, line, now),
				}
			}
			if s.buffer.Len() > 0 {
				s.buffer.WriteRune('\n')
	...
	count := func(m *multilineStreamsStriped) int {
		var n int
		for i := range m.stripes {
			m.stripes[i].mu.Lock()
			n += len(m.stripes[i].data)
			m.stripes[i].mu.Unlock()
		}
		return n
	}

	t.Run("Mutate returning nil does not store new state", func(t *testing.T) {
		m := newMultilineStreamsStriped()

		m.Mutate(1, func(s *multilineState, ok bool) *multilineState {
			require.Nil(t, s)
			require.False(t, ok)
			return nil
		})

		require.Equal(t, 0, count(m))
		require.Empty(t, m.FlushAll())
	})

	t.Run("Mutate returning nil keeps existing state", func(t *testing.T) {
		m := newMultilineStreamsStriped()
		buffer(m, 1, "START a", now)
		...
		require.Equal(t, 1, count(m))

		flushed := m.FlushAll()
		require.Len(t, flushed, 1)
		require.Equal(t, "START a", flushed[0].Line)
	})

	t.Run("fingerprints that share a stripe keep independent state", func(t *testing.T) {
		m := newMultilineStreamsStriped()

		// stripe() masks the fingerprint with stripeCount-1, so these collide.
		const (
			fp         = model.Fingerprint(3)
			sameStripe = model.Fingerprint(3 + stripeCount)
		)
		require.Same(t, m.stripe(fp), m.stripe(sameStripe))

		buffer(m, fp, "START a", now)
		buffer(m, sameStripe, "START b", now)
		require.Equal(t, 2, count(m))

		flushed := m.FlushAll()
		require.Len(t, flushed, 2)
		require.ElementsMatch(t, []string{"START a", "START b"}, []string{flushed[0].Line, flushed[1].Line})
	})

	t.Run("FlushOlderThan expires a state last seen exactly at the threshold", func(t *testing.T) {
		m := newMultilineStreamsStriped()
		buffer(m, 1, "at threshold", now)
		buffer(m, 2, "after threshold", now.Add(time.Nanosecond))

		expired := m.FlushOlderThan(now)
		require.Len(t, expired, 1)
		require.Equal(t, "at threshold", expired[0].Line)

		require.Equal(t, 1, count(m))

		remaining := m.FlushAll()
		require.Len(t, remaining, 1)
		require.Equal(t, "after threshold", remaining[0].Line)
	})

	t.Run("FlushOlderThan deletes an expired state that has no buffered lines", func(t *testing.T) {
		m := newMultilineStreamsStriped()
		buffer(m, 1, "START a", now)

		m.Mutate(1, func(s *multilineState, _ bool) *multilineState {
			s.flush()
			return s
		})
		require.Equal(t, 1, count(m))

		require.Empty(t, m.FlushOlderThan(now))
		require.Equal(t, 0, count(m))
	})

	t.Run("FlushAll drains every buffered state and clears the map", func(t *testing.T) {
		m := newMultilineStreamsStriped()
		buffer(m, 1, "one", now)
		buffer(m, 1, "one more", now)
		buffer(m, 2, "two", now)

		flushed := m.FlushAll()
		require.Len(t, flushed, 2)
		require.Equal(t, "one\none more", flushed[0].Line)
		require.Equal(t, "two", flushed[1].Line)
		require.Empty(t, m.FlushAll())
		require.Equal(t, 0, count(m))
	})

	t.Run("FlushAll clears a state with no buffered lines without emitting it", func(t *testing.T) {
		m := newMultilineStreamsStriped()
		buffer(m, 1, "START a", now)
