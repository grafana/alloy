...
func TestLabelsPipeline_LabelsFromStructuredMetadata(t *testing.T) {
	pl, err := NewPipeline(log.NewNopLogger(), loadConfig(testLabelsStrucuturedMetadataYaml), nil, prometheus.DefaultRegisterer, featuregate.StabilityGenerallyAvailable)
	if err != nil {
		t.Fatal(err)
	}
	expectedLbls := model.LabelSet{
