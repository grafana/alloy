...
// TestNHCBDeltaBuckets tests that Native Histograms with Custom Buckets (NHCB)
// using integer/delta-encoded buckets are properly converted to OTel Histograms
// with explicit bucket boundaries.
//
// NHCB histograms use schema -53 and store bucket counts as delta-encoded values
// in PositiveBuckets. The receiver should convert these to OTel Histograms
// (not ExponentialHistograms) with explicit bounds from CustomValues.
func TestNHCBDeltaBuckets(t *testing.T) {
	...
	go func() {
		...
		// Create an integer NHCB histogram with delta-encoded buckets
		// Schema -53 indicates custom buckets
		// CustomValues defines explicit bucket boundaries: [1.0, 2.0, 5.0, 10.0]
		// PositiveBuckets are delta-encoded: [10, 15, 20, 5, 0]
		//   -> Absolute counts: [10, 25, 45, 50, 50]
...
// TestNHCBAbsoluteBuckets tests that Native Histograms with Custom Buckets (NHCB)
// using float/absolute bucket counts are properly converted to OTel Histograms
// with explicit bucket boundaries.
//
// Float histograms store bucket counts as absolute values (not delta-encoded).
func TestNHCBAbsoluteBuckets(t *testing.T) {
	...
	go func() {
		...
		// Create a float NHCB histogram with absolute bucket counts
		// Schema -53 indicates custom buckets
		// CustomValues defines explicit bucket boundaries: [0.5, 2.0]
		// PositiveBuckets are absolute counts: [15.0, 20.0, 15.0]
