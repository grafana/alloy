...
func TestTranslateMetricsV1(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expect: func(t *testing.T, result pmetric.Metrics) {
				expectedResourceAttrs, expectedScopeAttrs, expectedDpAttrs := tagsToAttributes([]string{"env:tag1", "version:tag2"}, "Host1")
...
func TestTranslateMetricsV2(t *testing.T) {
	tests := []struct {
		name   string
		series []*gogen.MetricPayload_MetricSeries
		expect func(t *testing.T, result pmetric.Metrics)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mt := createMetricsTranslator()
			result := translateMetricsV2(tt.series, mt)
