...
// Package logging implements support for testing implementations of log/slog.Handler.
package logging

// NOTE THIS IS COPIED FROM GO AND PRIMARILY USED SINCE WE OVERRIDE THE NAME OF SOME FIELDS
...
var cases = []testCase{
	{
		name:        "built-ins",
		explanation: withSource("this test expects ts, slog.LevelKey and slog.MessageKey"),
	...
	},
	{
		...
		f: func(l *slog.Logger) {
			l.Info("msg", "k", &replaceAny{"replaced"})
		},
	...
	},
	{
		...
		f: func(l *slog.Logger) {
			l = l.With("k", &replaceAny{"replaced"})
			l.Info("msg")
		},
		checks: []check{hasAttr("k", "replaced")},
	},
...
// The standard keys ["ts"], [slog.LevelKey] and [slog.MessageKey] should be used.
//
...
// RunDeferredTests exercises a [slog.Handler] on the same test cases as [TestHandler], but
// runs each case in a subtest. For each test case, it first calls newHandler to
// get an instance of the handler under test, then runs the test case, then
// calls result to get the result. If the test case fails, it calls t.Error.
func RunDeferredTests(t *testing.T, newHandler func(*testing.T) *Logger, result func(*testing.T) map[string]any) {
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			// Dont run the modded tests
			if c.mod != nil {
				return
			}
			h := newHandler(t)

			l := slog.New(h.deferredSlog)
			c.f(l)
			err := h.Update(Options{
				Level:  "debug",
				Format: "json",
			})
...
func (r *replaceAny) LogValue() slog.Value { return slog.AnyValue(r.v) }

func (r *replaceAny) String() string {
